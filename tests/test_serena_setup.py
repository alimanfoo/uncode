...
class TestSetup:
    def _run(self, tmp_path, name="my-app"):
        # Write a real ``pyproject.toml`` at ``tmp_path`` so
        # ``setup(root=tmp_path)`` reads ``name`` through the production
        # ``read_project_name(start=root)`` path. Threading ``root`` into
        # ``read_project_name`` (GH68) is what makes mocking unnecessary
        # — without the threading, the cwd-walk would reach uncoded's
        # own ``pyproject.toml`` and the tests would have to mock around
        # that to assert anything name-dependent.
    ...
    def test_setup_uses_root_name_when_no_pyproject(self, tmp_path):
        ...
        # back to the start-dir name. cwd is *deliberately* not aligned
        # to ``tmp_path`` here: the contract under test (GH68) is that
        # ``setup(root=...)`` uses ``root`` for the project-name lookup
        # independently of cwd.
        setup(root=tmp_path)
        data = yaml.safe_load((tmp_path / ".serena" / "project.yml").read_text())
        assert data["project_name"] == tmp_path.name

    def test_setup_reads_name_from_pyproject(self, tmp_path):
        # End-to-end: ``setup(root=tmp_path)`` reads the name from a
        # ``pyproject.toml`` at ``tmp_path`` regardless of cwd. Before
        # the GH68 fix the cwd-walk could return uncoded's own project
        # name instead of ``"from-pyproject"``.
